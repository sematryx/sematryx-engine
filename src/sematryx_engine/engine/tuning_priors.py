"""Deterministic solver hyperparameter priors from domain label and problem-shape signals."""
...
def neutral_tuning_priors() -> dict[str, object]:
    """Shape- and domain-blind defaults used when the tuning-priors ablation is off.
...
def compute_solver_tuning_priors(
    *,
    ...
    regime_budget = {"generous": 1.05, "moderate": 1.0, "tight": 0.94}
    directive_population = {"local": 1.0, "balanced": 1.06, "aggressive": 1.12}

    cx = complexity_budget.get(features.complexity, 1.0)
    tb = regime_budget.get(budget_regime, 1.0)
    ...
    polish = not (budget_regime == "tight" and shape_routing_directive == "aggressive")

    pop_scale = directive_population.get(shape_routing_directive, 1.0)
