...
- Continuous-only `run_optimization` excludes discrete/hybrid strategy IDs from bandit selection so fresh CI environments cannot route bounds-only problems through `hybrid_outer_random_inner_scipy`.
